...
pub fn run() {
  ...
  let opt1 = grid.count_trees(1, 1);
  let opt2 = grid.count_trees(3, 1);
  let opt3 = grid.count_trees(5, 1);
  let opt4 = grid.count_trees(7, 1);
  let opt5 = grid.count_trees(1, 2);

  println!("Answer: {}", opt1 * opt2 * opt3 * opt4 * opt5);
}
